...
- v1.0.0: Stabilization and production-hardening release. Operator profile presets (`safe_default`, `continuity_optimized`, `mediation_enabled`, `workflow_limited`) added for configuration clarity. `RuntimeStatusInspector` and `agent_get_runtime_status` MCP tool added for resolved runtime state inspection. `AuditPresenter` helpers added for structured observability. Warning messages normalized with stable category codes (`[tool_downgrade]`, `[mediation_rejected]`, etc.). `reconstruction_version` updated to `"v1.0.0"`. No breaking changes to MCP contracts or response envelopes. All v0.6–v0.9 deployments are fully forward-compatible.
